...
class LoadStreams:  # multiple IP or RTSP cameras
    def __init__(self, sources='streams.txt', img_size=640):
        ...
        for i, s in enumerate(sources):
            ...
            #cap = cv2.VideoCapture(eval(s) if s.isnumeric() else s)#,cv2.CAP_DSHOW)
            cap = cv2.VideoCapture(6)#,cv2.CAP_DSHOW)
            
            print("set camera cameara")
    ...
    def __next__(self):
        ...
        #img = [letterbox(x, new_shape=(896,896), auto_size=64,auto = False,scaleFill=True)[0] for x in img0]
