...
class TomatoPlanner(object):
    def __init__(self):
        ...
        self.robotState = "waiting" # waiting, moving, serching, picking
    ...
    def process(self):
        rospy.init_node('tomatoFinder', anonymous=True)
        #r = rospy.Rate(60)
        ...
        while not rospy.is_shutdown():
            print("Find Tomato  is Ready...")
            sign = rospy.wait_for_message("search_direction",Float64)
            ...
            try:
                while time.time() - t0 < 10:
                    ...
                    if self.fullRedTomato.num != 0 and abs(self.fullRedTomato.camPoseArray[0][0]) > 0.001:
                        ...
                        clippedX = max(-0.5, min(0.5, tomatoX))
                        updateVel = -1 * sign * np.interp(clippedX,[-0.5,0.5],[-0.2,0.2])
