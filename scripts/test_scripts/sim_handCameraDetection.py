...
class DepthImageHandler(object):
    def __init__(self):
        self.CAMINFO = {'topic': '/hand_rsd435/color/camera_info', 'msg': CameraInfo}
        self.isCamInfo = False

        self.COLOR = {'topic': '/hand_rsd435/color/image_raw', 'msg': Image}
        self.DEPTH = {'topic': '/hand_rsd435/depth/image_raw', 'msg': Image}
    ...
    def pointToDepthUV(self, point3):
        ...
        u = (-y*self.K[0]/x) + self.K[2]/self.cropSize
        v = (-z*self.K[4]/x) + self.K[5]/self.cropSize   
        return [int(u),int(v)]
    ...
    def publishPoint3(self,pos3):
        ...
        point.header.frame_id = "hand_rgb"
    ...
    def GetTomato(self):
        ...
        collisionCheckBlock = self.pixel_crop(color_image,[50,50],self.suckUV)
        cv2.circle(visual_color_image,(self.suckUV[0],self.suckUV[1]), 20, (0,0,255), -1)
        # imgplot = plt.imshow(depth_image)
        # plt.show()
        #print(depth_image.dtype)
        cv2.imshow("image", visual_color_image)
        # cv2.imshow("mask", masked_depth)
        #cv2.imshow("tomato", masked_color)
        cv2.imshow("heta", collisionCheckBlock)
        cv2.waitKey(1)
    ...
    def findTomatoCallBack(self,req):
        ...
        return SelectTomatoResponse(collisionArray)
