...
class YOLOR(object):
    def __init__(self,conf=0.7):
        ...
        self.model = Darknet(CONFIG_PATH, self.imgsz).cuda()
...
class DepthImageHandler(object):
    ...
    def depthPixelToPoint3(self, depth_image, U, V):
        ...
        z = meanDepth/1000 # /1000 for real camera
        x *= z
        y *= z
        # print(U,V,x,y,z)
        point3 = [x, y, z]
    ...
    def pos3FromBboxes(self, depthImage, bboxes:list)->list:
        ...
        for (x1,y1,x2,y2) in bboxes:
            ...
            bboxList = [x1,y1,x2,y2]
            bboxList = Float32MultiArray(data = bboxList)
            print(bboxList)
            self.maskimg_pub.publish(bboxList) ##publish of bboxList
        return pos3List
    ...
    def process(self):
        ...
        print(f'camPosDict is {camPosDict}')
        # self.maskimg_pub.publish(json.dumps(bboxDict)) ##publish of bboxDict
    ...
    def rosinit(self):
        ...
        self.object_pub     = rospy.Publisher( '/object/spaghetti',    Marker , queue_size=10)
        self.maskimg_pub    =  rospy.Publisher( '/object/masking',    Float32MultiArray , queue_size=10) #publisher of bboxList
